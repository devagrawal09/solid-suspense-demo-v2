...
function children(fn) {
  ...
  const memo = signals.createMemo(() => resolveChildren(children()));
  memo.toArray = () => {
    const c = memo();
...
function ErrorBoundary(props) {
  let err;
  const [errored, setErrored] = signals.createSignal(err);
  Errors || (Errors = new Set());
  Errors.add(setErrored);
  signals.onCleanup(() => Errors.delete(setErrored));
  return signals.createMemo(() => {
    let e;
    if (e = errored()) {
      const f = props.fallback;
      return typeof f === "function" && f.length ? signals.untrack(() => f(e, () => setErrored())) : f;
    }
    return signals.catchError(() => props.children, setErrored);
  });
}
...
function template(html, isImportNode, isSVG) {
  ...
  return isImportNode ? () => signals.untrack(() => document.importNode(node || (node = create()), true)) : () => (node || (node = create())).cloneNode(true);
}
...
function spread(node, props = {}, isSVG, skipChildren) {
  const prevProps = {};
  if (!skipChildren) {
    signals.createRenderEffect(() => normalize(props.children, prevProps.children), value => {
      insertExpression(node, value, prevProps.children);
      prevProps.children = value;
    });
  }
  signals.createRenderEffect(() => typeof props.ref === "function" && use(props.ref, node), () => ({}));
  signals.createRenderEffect(() => {
    const newProps = {};
...
function use(fn, element, arg) {
  return signals.untrack(() => fn(element, arg));
}
function insert(parent, accessor, marker, initial) {
  const multi = marker !== undefined;
  if (multi && !initial) initial = [];
  if (typeof accessor !== "function") return insertExpression(parent, normalize(accessor, initial, multi), initial, marker);
  signals.createRenderEffect(prev => normalize(accessor, prev, multi), (value, current) => insertExpression(parent, value, current, marker), initial);
}
...
function normalize(value, current, multi) {
  ...
  if (t === "number") value = value.toString();else if (t === "boolean" || value == null || value === "") value = undefined;else if (Array.isArray(value)) {
    const normalized = [];
...
function normalizeIncomingArray(normalized, array, current) {
  for (let i = 0, len = array.length; i < len; i++) {
    let item = array[i],
      ...
      t;
    if (item == null || item === true || item === false || item === "") ; else if ((t = typeof item) === "object" && item.nodeType) {
