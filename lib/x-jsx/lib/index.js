...
function ErrorBoundary(props) {
  ...
  return signals.createErrorBoundary(() => props.children, (err, reset) => {
    Errors.add(reset);
    signals.onCleanup(() => Errors.delete(reset));
...
function lazy(fn) {
  ...
  const wrap = props => {
    ...
    let Comp;
    return signals.createMemo(() => (Comp = comp()) ? signals.untrack(() => Comp(props)) : "");
  };
...
function use(fn, element, arg) {
  signals.untrack(() => fn(element, arg));
}
